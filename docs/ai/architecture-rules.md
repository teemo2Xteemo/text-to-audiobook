...
**Resolution of §2 vs §3/§6/§40:** §2’s “Chinese story / Vietnamese TTS” is the primary demo path. Domain code uses `source_language` / `target_language`. UI defaults (if any) belong in **config**, e.g. `VITE_DEFAULT_TARGET_LANGUAGE` (requirements/ADR `DEFAULT_TARGET_LANGUAGE`), not in services. MVP source preselect is `auto`, not `zh`.
...
| `DEFAULT_SOURCE_LANGUAGE=zh` in requirements §27 | Allowed **as operator env/config** (`VITE_DEFAULT_SOURCE_LANGUAGE` in the Vite client), never as domain or TypeScript constants. MVP default remains `auto`, not `zh`. |
